...
//! @{
//! \file PersistentMap.cpp
//! \brief Implementation of the nand::PersistentMap class.
////////////////////////////////////////////////////////////////////////////////
...
//! \brief Macro used to help print the map signature as a four-char constant.
...
void PersistentMap::init(int entrySize, int entryCount)
{
...
////////////////////////////////////////////////////////////////////////////////
//! \brief   This function retrieves a section of the map from nand.
//!
//! This function retrieves a section of the map which contains the entry
//! number u32EntryNum.
//!
//! \param[in] u32BlockNum  Block number containing the zone map.
//! \param[in] u32TotalSectors  Number of pages written to the zone map block
//!     thus far.
//! \param[in]  u32EntryNum          Entry number which should be contained
//!                                  in section of zone-map returned.
//! \param[out] pMap Section of Zone-map which contains the entry number
//!     contained in u32EntryNum. This buffer will be filled with the entire
//!     contents of the map section page, including the header.
//! \param mapType Constant to compare against the \a mapType field of the
//!     map section header. If the two do not match, then the section is
//!     considered to be corrupted.
//! \param[in] shouldConsolidateOnRewriteSectorError Pass true to have the
//!     function automatically consolidate the zone map, i.e. rewrite it to
//!     a new block, if the ECC_FIXED_REWRITE_SECTOR error is returned while
...
//    int32_t    i;
    ...
    else
    {
        ...
        // Scan the zone map block backwards, looking for the most recent copy of the requested section.
//        for (i = (m_topPageIndex - 1); i >= 0; i--)
//        {
//            // Read the page.
//            sectionPage = PageAddress(m_block, i);
//            retCode = sectionPage.read();
//        
//            // If we got back valid data, first check to make sure this section is really what we expect.
//            if (is_read_status_success_or_ecc_fixed(retCode))
...
//            }
//
//            // Exit the loop immediately if we need to consolidate because of ECC hitting the
//            // threshold.
...
//            else
//            {
//                // Exit loop for other random errors.
//                break;
//            }
//
...
//            if ((header->startLba <= u32EntryNum) && (header->startLba + header->entryCount > u32EntryNum))
//            {
//                // Update the page offset array.
//                m_sectionPageOffsets.setEntry(sectionNumber, i);
//                break;
//            }
//        }
//
//        // Make sure we found the section we were looking for.
//        if (i < 0)
//        {
//            retCode = ERROR_DDI_NAND_MAPPER_LBA_CORRUPTED;
//        }
    ...
    if (retCode == ERROR_DDI_NAND_HAL_ECC_FIXED_REWRITE_SECTOR && shouldConsolidateOnRewriteSectorError)
    {
        tss_logtext_Print(LOGTEXT_EVENT_DDI_NAND_GROUP|LOGTEXT_VERBOSITY_1, ">>> Got ECC_FIXED_REWRITE_SECTOR error reading page %d of map block %d\n", pageOffset, m_block);
        tss_logtext_Print(LOGTEXT_EVENT_DDI_NAND_GROUP|LOGTEXT_VERBOSITY_1, ">>> Rewriting by consolidating map '%c%c%c%c'\n", SIG_AS_4CHARS());
...
RtStatus_t PersistentMap::buildSectionOffsetTable()
{
...
#if LOG_MAP
    tss_logtext_Print(LOGTEXT_EVENT_DDI_NAND_GROUP|LOGTEXT_VERBOSITY_1, "Took %u reads to find top page (%u) for map '%c%c%c%c'\n", m_buildReadCount, m_topPageIndex, SIG_AS_4CHARS());
#endif
    ...
    if (needsRewrite)
    {
        tss_logtext_Print(LOGTEXT_EVENT_DDI_NAND_GROUP|LOGTEXT_VERBOSITY_1, "Rewriting map '%c%c%c%c' after building section offset table\n", SIG_AS_4CHARS());
        return consolidate();
    }
    
#if LOG_MAP
    tss_logtext_Print(LOGTEXT_EVENT_DDI_NAND_GROUP|LOGTEXT_VERBOSITY_1, "Took %u reads to build map '%c%c%c%c'\n", m_buildReadCount, SIG_AS_4CHARS());
#endif

    return SUCCESS;
}

RtStatus_t PersistentMap::findTopPageIndex(ZoneMapSectionPage & mapPage, bool & needsRewrite)
{
    RtStatus_t status;
    unsigned i = 0;
    ...
    while (highestFilled < lowestErased)
    {
        ...
        if (status == ERROR_DDI_NAND_HAL_ECC_FIXED_REWRITE_SECTOR)
        {
            ...
            // before then.
            needsRewrite = true;
            tss_logtext_Print(LOGTEXT_EVENT_DDI_NAND_GROUP|LOGTEXT_VERBOSITY_1, "Got ECC_FIXED_REWRITE_SECTOR while reading page %d; will rewrite later\n", mapPage.get());
        }
        ...
        else
        {
            ...
            // Go ahead and update the page offset array. No reason to have to read
            // this page again in the loop below.
//            m_sectionPageOffsets.setEntry(mapPage.getSectionNumber(), i);
        }
...
#if LOG_MAP
    tss_logtext_Print(LOGTEXT_EVENT_DDI_NAND_GROUP|LOGTEXT_VERBOSITY_1, "Map '%c%c%c%c' block %d has %d filled pages\n", SIG_AS_4CHARS(), m_block.get(), i);
#endif
    
    return SUCCESS;
}

RtStatus_t PersistentMap::fillUnknownSectionOffsets(ZoneMapSectionPage & mapPage, bool & needsRewrite)
{
    RtStatus_t status;
    ...
    for (; i >= 0; --i, --mapPage)
    {
        ...
        if (distinctEntries == m_totalSectionCount)
        {
            return SUCCESS;
        }
...
////////////////////////////////////////////////////////////////////////////////
//! \brief This function consolidates the zone-map.
//!
...
//!
//! \param[in] hasValidSectionData Whether \a sectionData is valid.
//! \param[in] sectionNumber The number of the zone map section for which
//!     \a sectionData contains data. Ignored if \a hasValidSectionData is false.
//! \param[in] sectionData Zone map data for the map section identified by
//!     \a sectionNumber. Ignored if \a hasValidSectionData is false. This data
//!     must \em not have the map section header in it. In other words, the data
//!     should be only an array of entries. The metadata that is contained in the
//!     header on the media is passed in the \a sectionNumber and \a sectionDataEntryCount
//!     variables.
//! \param[in] sectionDataEntryCount Number of entries in \a sectionData.
//!
//! \return Status of call or error.
//! \retval SUCCESS If no error has occurred.
////////////////////////////////////////////////////////////////////////////////
    ...
    uint32_t sectionDataEntryCount)
{
    ...
    ret = m_mapper.getBlock(&newMapBlockNumber, kMapperBlockTypeMap, NULL);
    if (SUCCESS != ret)
    {
        return ret;
    }
    ...
    tss_logtext_Print(LOGTEXT_VERBOSITY_1 | LOGTEXT_EVENT_DDI_NAND_GROUP, "Consolidating map '%c%c%c%c' from block %u to block %u\n", SIG_AS_4CHARS(), m_block.get(), newMapBlockNumber);
    ...
    while (u32EntryNum < m_totalEntryCount)
    {
        uint8_t * bufferToWrite;
        uint32_t bufferEntryCount;
        uint32_t thisSectionNumber = u32EntryNum / m_maxEntriesPerPage;
        
        // First check if we can use the data passed into this function. It has preference over
        // any other source (cache or media).
        ...
        else
        {
            ...
            if (ret == ERROR_DDI_NAND_HAL_ECC_FIX_FAILED && !m_mapper.isBuildingMaps())
            {
                ...
                tss_logtext_Print(LOGTEXT_EVENT_DDI_NAND_GROUP|LOGTEXT_VERBOSITY_1, ">>> Got uncorrectable ECC error reading map section; rebuilding maps\n");
            ...
            {
                tss_logtext_Print(LOGTEXT_EVENT_DDI_NAND_GROUP|LOGTEXT_VERBOSITY_1, ">>> getSectionForConsolidate(%d, %d, %d) failed during consolidate(): 0x%08x\n", m_block.get(), m_topPageIndex, u32EntryNum, ret);
        ...
        {
            tss_logtext_Print(LOGTEXT_EVENT_DDI_NAND_GROUP|LOGTEXT_VERBOSITY_1, ">>> Got write error for page %d while consolidating: 0x%08x; marking bad\n", sectionPage.get(), ret);
            
            // Process the new bad block.
            m_mapper.handleNewBadBlock(targetBlock);
            
            // We need to Consolidate into another block.
            // Reset variables so that the loop will start over from
            // beginning with a new block.
            ret = m_mapper.getBlock(&newMapBlockNumber, kMapperBlockTypeMap, NULL);
            if (SUCCESS != ret)
            {
                return ret;
...
////////////////////////////////////////////////////////////////////////////////
//! \brief   This function adds a zone-map section to current block.
//!
...
//!
//! \param[in] pMap Pointer to contents of the map section which contains
//!     the entry number contained in u32EntryNum.
//! \param[in] u32StartingEntryNum  The starting entry number for the given section.
//! \param[in] u32NumEntriesToWrite Number of entries contained in this section.
//!     This will usually, but not always, be the maximum number of entries per page.
//!
//! \return Status of call or error.
//! \retval SUCCESS If no error has occurred.
////////////////////////////////////////////////////////////////////////////////
RtStatus_t PersistentMap::addSection(uint8_t *pMap, uint32_t u32StartingEntryNum, uint32_t u32NumEntriesToWrite)
{
    uint32_t sectionNumber;
...
#if LOG_MAP
    tss_logtext_Print(LOGTEXT_VERBOSITY_1 | LOGTEXT_EVENT_DDI_NAND_GROUP, "Adding section (entry %u+%u) to map '%c%c%c%c' block %u in page %u\n", u32StartingEntryNum, u32NumEntriesToWrite, SIG_AS_4CHARS(), m_block.get(), m_topPageIndex);
#endif
    ...
    assert(m_topPageIndex <= pPhyMediaDescriptor->pNANDParams->wPagesPerBlock);
    
    // Compute the index into the section page offset array.
    sectionNumber = u32StartingEntryNum / m_maxEntriesPerPage;

    u32MaxNumEntries = m_maxEntriesPerPage;
    u32NumEntriesToWrite = std::min(u32NumEntriesToWrite, u32MaxNumEntries);
    u32NumEntriesToWrite = std::min(u32NumEntriesToWrite, (m_totalEntryCount - u32StartingEntryNum));
    ...
    // consolidate.
    if (m_topPageIndex >= pPhyMediaDescriptor->pNANDParams->wPagesPerBlock)
    {
    ...
    else
    {
        ...
        if (ret == ERROR_DDI_NAND_HAL_WRITE_FAILED)
        {
            ...
            tss_logtext_Print(LOGTEXT_EVENT_DDI_NAND_GROUP|LOGTEXT_VERBOSITY_1, ">>> Got write error for block %d page %d while adding map '%c%c%c%c' section: 0x%08x; consolidating and marking bad\n", m_block.get(), m_topPageIndex, SIG_AS_4CHARS(), ret);
